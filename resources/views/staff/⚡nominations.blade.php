...
new class extends Component {
    ...
    // Social Links
    #[Validate('nullable|url|max:255')] public $facebook_url = '';
    #[Validate('nullable|url|max:255')] public $instagram_url = '';
    #[Validate('nullable|url|max:255')] public $twitter_url = '';
    #[Validate('nullable|url|max:255')] public $tiktok_url = '';
    #[Validate('nullable|url|max:255')] public $youtube_url = '';
    #[Validate('nullable|url|max:255')] public $website_url = '';
    ...
    {
        return [
            'code' => 'required|unique:nominations,code,' . $this->nominationId,
        ];
    }
    ...
    public function create()
    {
        ...
        $this->reset([
            ...
            'twitter_url', 'tiktok_url', 'youtube_url', 'website_url', 'is_active', 'isEditMode'
        ]);
        $this->generateCode();
        $this->showModal = true;
    }
    ...
    public function edit($id)
    {
        ...
        $this->is_active = $nomination->is_active;
    ...
    public function removeExistingProfileImage()
    {
    ...
    public function save()
    {
        ...
        if ($this->profile_image) {
            ...
            $oldNom = Nomination::find($this->nominationId);
            if($oldNom && $oldNom->profile_image) {
                Storage::disk('public')->delete($oldNom->profile_image);
            }
            $imagePath = null;
        }
        ...
        Nomination::updateOrCreate(
            ...
            [
                ...
                'name' => $this->name,
                'code' => $this->code,
                'company_or_show' => $this->company_or_show,
        ...
        session()->flash('message', $this->isEditMode ? 'Nominee updated successfully.' : 'Nominee added successfully.');
    }
...
<div class="max-w-7xl mx-auto text-gray-200">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-400 mt-1">Manage the candidates, their voting codes, and social profiles.</p>
        </div>
    ...
    <div class="bg-gray-800 rounded-xl shadow-xl border border-gray-700 overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-700">
                        @foreach($this->nominations as$nominee)
                            <tr class="hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-4">
                                        @if($nominee->profile_image)
                                            <img class="h-10 w-10 rounded-full object-cover border border-gray-600 shadow-sm" src="{{ asset('storage/' . $nominee->profile_image) }}" alt="">
                                        @else
                                            <div class="h-10 w-10 rounded-full bg-gray-700 border border-gray-600 flex items-center justify-center">
                                                <svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                            </div>
                                        @endif
                                        <div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 bg-gray-900 border border-gray-600 rounded text-xs text-yellow-500 font-mono">{{ $nominee->code }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300 font-bold">
                                    {{ number_format($nominee->total_votes) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button wire:click="toggleActive({{ $nominee->id }})" class="px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full transition-colors {{ $nominee->is_active ? 'bg-green-500/10 text-green-400 border border-green-500/20 hover:bg-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20' }}">
                                        {{ $nominee->is_active ? 'Active' : 'Inactive' }}
                                    </button>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button wire:click="edit({{ $nominee->id }})" class="text-yellow-500 hover:text-yellow-400 mr-4 transition-colors">Edit</button>
                                    <button wire:click="delete({{ $nominee->id }})" wire:confirm="Are you sure you want to remove this nominee?" class="text-red-500 hover:text-red-400 transition-colors">Delete</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center py-20 px-4 text-center">
                ...
                <p class="text-gray-400 max-w-sm mb-6">Start populating your categories by adding the first nominee. Ensure you've created categories first.</p>
    ...
    <!-- Create/Edit Modal (Wide Layout for More Data) -->
    <div x-data="{ show: @entangle('showModal') }" x-show="show" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="inline-block w-full max-w-2xl my-8 overflow-hidden text-left align-middle transition-all transform bg-gray-800 border border-gray-700 rounded-xl shadow-2xl shadow-black/50">
                <form wire:submit="save">
                    <div class="px-6 py-5 border-b border-gray-700 bg-gray-900/30 flex justify-between items-center sticky top-0 z-10">
                    ...
                    <div class="px-6 py-5 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <select wire:model="nomination_category_id" class="mt-1 block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-white focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2.5">
                            ...
                            <div>
                                ...
                                <div class="flex mt-1 rounded-lg shadow-sm">
                                    <input type="text" wire:model="code" class="block w-full bg-gray-900 border border-gray-600 rounded-l-lg text-white focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2.5 font-mono uppercase">
                                    <button type="button" wire:click="generateCode" class="px-4 py-2 bg-gray-700 border border-gray-600 border-l-0 rounded-r-lg text-sm text-gray-300 hover:text-white hover:bg-gray-600 transition-colors">
                                        Regenerate
                                    </button>
                                </div>
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="name" class="mt-1 block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-white focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2.5">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="company_or_show" placeholder="e.g. Citizen TV" class="mt-1 block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-white focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2.5">
                                @error('company_or_show') <span class="text-xs text-red-400 mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>

                        <!-- Image Upload -->
                        <div class="border-t border-gray-700 pt-5" x-data="{ isUploading: false, progress: 0 }"
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                @if ($profile_image)
                                    <div class="relative inline-block group shrink-0">
                                        <img src="{{ $profile_image->temporaryUrl() }}" class="h-20 w-20 object-cover rounded-full border-2 border-yellow-500 shadow-md">
                                        <button type="button" wire:click="removeProfileImage" class="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-1 shadow-lg hover:bg-red-600 hover:scale-110 transition-transform">
                                ...
                                @elseif ($existingProfileImage)
                                    <div class="relative inline-block group shrink-0">
                                        ...
                                        <button type="button" wire:click="removeExistingProfileImage" class="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-1 shadow-lg hover:bg-red-600 hover:scale-110 transition-transform opacity-0 group-hover:opacity-100">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
                                        </button>
                                    </div>
                                @else
                                    <div class="h-20 w-20 rounded-full bg-gray-900 border border-gray-600 flex items-center justify-center shrink-0">
                                        <svg class="w-8 h-8 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                    </div>
                                @endif

                                <div class="w-full">
                                    <label for="dropzone-file-nom" class="flex flex-col items-center justify-center w-full h-20 border-2 border-gray-600 border-dashed rounded-lg cursor-pointer bg-gray-900 hover:bg-gray-700/50 hover:border-yellow-500 transition-all">
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                            <p class="text-sm text-gray-400"><span class="font-semibold text-yellow-500">Upload Image</span></p>
                                        </div>
                                        <input id="dropzone-file-nom" type="file" wire:model="profile_image" class="hidden" accept="image/*" />
                                    </label>
                                    <div x-show="isUploading" style="display: none;" class="w-full bg-gray-700 rounded-full h-1 mt-2 overflow-hidden">
                                        <div class="bg-yellow-500 h-1 rounded-full transition-all duration-300" x-bind:style="`width: ${progress}%`"></div>
                                    </div>
                        ...
                        <div class="border-t border-gray-700 pt-5">
                            ...
                            <textarea wire:model="bio" rows="3" class="mt-1 block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-white focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2.5"></textarea>
                        ...
                        <div class="border-t border-gray-700 pt-5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <input type="text" wire:model="facebook_url" placeholder="Facebook URL" class="block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-gray-300 focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2">
                                    @error('facebook_url') <span class="text-xs text-red-400 mt-1 block">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <input type="text" wire:model="instagram_url" placeholder="Instagram URL" class="block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-gray-300 focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2">
                                    @error('instagram_url') <span class="text-xs text-red-400 mt-1 block">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <input type="text" wire:model="twitter_url" placeholder="Twitter (X) URL" class="block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-gray-300 focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2">
                                    @error('twitter_url') <span class="text-xs text-red-400 mt-1 block">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <input type="text" wire:model="tiktok_url" placeholder="TikTok URL" class="block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-gray-300 focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2">
                                    @error('tiktok_url') <span class="text-xs text-red-400 mt-1 block">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <input type="text" wire:model="youtube_url" placeholder="YouTube URL" class="block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-gray-300 focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2">
                                    @error('youtube_url') <span class="text-xs text-red-400 mt-1 block">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <input type="text" wire:model="website_url" placeholder="Website URL" class="block w-full bg-gray-900 border border-gray-600 rounded-lg shadow-sm text-gray-300 focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm px-4 py-2">
                    ...
                    <div class="px-6 py-4 bg-gray-900/50 border-t border-gray-700 sm:flex sm:flex-row-reverse gap-3 rounded-b-xl">
